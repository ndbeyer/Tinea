...
const PortalContent = React.memo(
  ({
    ...
    const handleDismiss = React.useCallback(
      () => globalThis.portalRef?.unmount(id),
      [id],
    );
    if (
      !(
        globalThis.portalRef && componentName in globalThis.portalRef.components
      )
    ) {
...
PortalProvider.render = (
  ...
  if (!componentName)
    throw new Error('Portal components need a name attribute');
...
PortalProvider.unmount = (id: string): void =>
  globalThis.portalRef?.unmount(id);
