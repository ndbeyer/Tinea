...
const createContext = ({ req }): Context => {
	...
	const jwtToken =
		autHeader && autHeader.includes('Bearer ') ? autHeader.replace('Bearer ', '') : undefined;
...
(async () => {
	...
	// @ts-ignore
	await new Promise((resolve) => httpServer.listen({ port: CONSTS.PORT }, resolve as () => void));
	console.log(`>>> 🚀 Server ready at ${CONSTS.API_BASE_URL}${server.graphqlPath}`);
})();
